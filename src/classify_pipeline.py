...
class SimpleTransformerComments(TransformerMixin):
    ...
    def transform(self, X, **transform_params):
        ...
        # ls = X['words']
        print(X['words'].iloc[0])
        X['words'] = X['words'].apply(lambda x: " ".join(str(y) for y in x))
        # X['words'].apply(lambda x: x)
        print(X['words'].iloc[0])
        ls = X['words']
        print(ls.shape)
        return ls
...
clf = Pipeline([
    ('all_features', SimpleTransformerGeneral()),
    # ('ngrams', FeatureUnion([
    #
    #     # ('ngrams_tfidf', Pipeline([
    #     #     ('comments', SimpleTransformerComments()),
